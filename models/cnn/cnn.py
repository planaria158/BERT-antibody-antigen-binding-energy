...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(self.bn1(self.gelu(self.conv1(x))))  # 23x23
        x = self.pool(self.bn2(self.gelu(self.conv2(x)))) # 11x11
        x = self.pool(self.bn3(self.gelu(self.conv3(x))))  # 5x5
        x = self.pool(self.bn4(self.gelu(self.conv4(x))))  # 2x2
